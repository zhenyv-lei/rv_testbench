...
static uint8_t rv64_alloc_mem[PROBE_BYTES + EVICTION_BYTES + (2u * L1_SET_SPAN_BYTES)]
  __attribute__((aligned(L1_SET_SPAN_BYTES)));
static uintptr_t rv64_alloc_next;
...
{
  uintptr_t raw = (uintptr_t)rv64_alloc_mem + rv64_alloc_next;
  const uintptr_t aligned = (raw + alignment - 1u) & ~(uintptr_t)(alignment - 1u);
  const uintptr_t end = aligned + size;
  const uintptr_t pool_end = (uintptr_t)rv64_alloc_mem + sizeof(rv64_alloc_mem);

  if (end > pool_end)
    return NULL;

  rv64_alloc_next = end - (uintptr_t)rv64_alloc_mem;
  return (void *)aligned;
}
